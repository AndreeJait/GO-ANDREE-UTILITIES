...
func (c *redisClusterClient) MSetWithExpiration(keys []string, values []interface{}, ttls []time.Duration) error {
...
func (c *redisClusterClient) MSet(keys []string, values []interface{}) error {
	if err := check(c); err != nil {
		return errors.WithStack(err)
	}
...
func (c *redisClusterClient) SetNx(key string, value interface{}, ttl time.Duration) (bool, error) {
	if err := check(c); err != nil {
...
func (c *redisClusterClient) HDel(key string, fields ...string) error {
	if err := check(c); err != nil {
		return err
	}
...
func (c *redisClusterClient) ZIncrBy(key string, increment float64, member string) (float64, error) {
	if err := check(c); err != nil {
		return 0, errors.WithStack(err)
	}

	nx := c.r.ZIncrBy(key, increment, member)
	return nx.Result()
}

func (c *redisClusterClient) TTL(key string) (duration time.Duration, err error) {
	if err = check(c); err != nil {
		return duration, err
	}
...
func (c *redisClusterClient) Incr(key string) error {
	if err := check(c); err != nil {
		return errors.WithStack(err)
	}
...
func (c *redisClusterClient) IncrBy(key string, value int64) error {
	if err := check(c); err != nil {
		return errors.WithStack(err)
	}
