...
func (c *redisClient) MSetWithExpiration(keys []string, values []interface{}, ttls []time.Duration) error {
...
func (c *redisClient) MSet(keys []string, values []interface{}) error {
	if err := check(c); err != nil {
		return errors.WithStack(err)
	}
...
func (c *redisClient) HDel(key string, fields ...string) error {
	if err := check(c); err != nil {
		return err
	}
...
func (c *redisClient) TTL(key string) (duration time.Duration, err error) {
	if err = check(c); err != nil {
		return duration, err
	}
...
func (c *redisClient) IncrBy(key string, value int64) error {
	if err := check(c); err != nil {
		return errors.WithStack(err)
	}
